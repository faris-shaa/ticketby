...
<body>
	<div class="container my-5">
      <div class="col-lg-12 px-10" style="overflow-wrap: break-word;"> 
	<div>
...
if (window.ApplePaySession) {
   var merchantIdentifier = 'merchant.com.wasltec.Applepaywasltec';
...
document.getElementById("applePay").onclick = function(evt) {
	 ...
	 var runningTotal	= function() { return <?php echo amount; ?> ; }
	 var shippingOption = "";
	...
	session.onvalidatemerchant = function (event) {
		logit(event , 'on validate merchant event' );
	...
	function performValidation(valURL) {
		var validate = new Promise(function(resolve, reject) {
			...
			xhr.onload = function() {
				...
				logit(data  , ' per form Validation response data' );
				resolve(data);
			};
			xhr.onerror = reject;
		...
		logit(validate , 'per form Validation');
		return validate;
	}
	...
	session.onshippingcontactselected = function(event) {
		...
		logit(event , 'on shipping contact selected event ');
	...
	session.onshippingmethodselected = function(event) {
		...
		logit(event , 'on shipping method selected event');
	...
	session.onpaymentmethodselected = function(event) {
		...
		logit(event , 'on payment method selected event' );
	...
	session.onpaymentauthorized = function (event) {
		...
		logit(event , 'on payment authorized event');
	...
	function sendPaymentToken(paymentToken) {
			var xhr = new XMLHttpRequest();
			logit('here is adnan.hashmi');
			var url = 'virtual.php?testkey=val&paymentToken=' + encodeURIComponent(JSON.stringify(paymentToken) );

			logit(encodeURIComponent(JSON.stringify(paymentToken) ));

			xhr.open( 'GET', url);
			xhr.onload = function() {
				logit(xhr.response , 'paymentToken' );
		...
		return new Promise(function(resolve, reject) {
			...
			logit(paymentToken , 'paymentToken' );
...
function logit(data , title = '') {
			if (debug == true) {
				console.log(data);
			}

			if (typeof data === 'string' || data instanceof String){
				//var old_data = document.getElementById("debug_message").innerHTML
				//document.getElementById("debug_message").innerHTML = data + '</br></br>' + old_data;
			}else{
				var old = console.log;
				var logger = document.getElementById('debug_log');

				if( title !== '' ){
					logger.innerHTML += '<h4>' + title + '</h4>';
				}
