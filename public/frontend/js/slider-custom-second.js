...
function slide(event) {
    ...
    if (event.currentTarget === arrow_right_second) {
        if (Math.abs(current_property_left_slider) + interval_second + slider_second.clientWidth >= slider_second.scrollWidth) {
             
            new_pos_left_slider = `-${slider_second.scrollWidth - slider_second.clientWidth}px`;
        } else {
